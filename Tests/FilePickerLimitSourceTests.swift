...
final class FilePickerLimitSourceTests: XCTestCase {
    func testFilePickerWarnsWhenRepositoryFileListIsTruncated() throws {
        ...
        XCTAssertTrue(source.contains("if didHitFileLimit && !searchText.isEmpty { return \"未匹配到已加载文件\" }"))
        XCTAssertTrue(source.contains("if didHitFileLimit && !searchText.isEmpty { return \"当前只搜索已加载的前 \\(maxFilesToLoad) 个文件；如果没找到，请缩小工作目录或直接输入 @file: 绝对路径\" }"))
